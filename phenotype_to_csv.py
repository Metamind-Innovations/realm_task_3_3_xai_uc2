...
def process_pharmcat_folders(base_dir='pharmcat_processed'):
    ...
    for sample_folder in sample_folders:
        ...
        # Find all phenotype.json files in the sample folder
        phenotype_files = glob.glob(os.path.join(sample_path, "*.phenotype.json"))

        for phenotype_file in phenotype_files:
            csv_path = os.path.splitext(phenotype_file)[0] + '.csv'

            # Convert phenotype.json to CSV
            output_csv = phenotype_to_csv(phenotype_file, csv_path)
            processed_files.append(output_csv)
            print(f"Converted {phenotype_file} to {output_csv}")
